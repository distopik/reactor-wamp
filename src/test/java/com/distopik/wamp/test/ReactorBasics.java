			...
			public void configure(WebSocketServletFactory factory) {
				factory.setCreator((req, resp) -> {
					...
					return new WebSocketAdapter() {
						...
						Broadcaster<Message>   messages = Broadcaster.<Message>create(Environment.cachedDispatcher());
						
						Map<Selector<String>, Registration<Consumer<? extends Event<?>>>> subs = new HashMap<>();
						
						private boolean verifySecurity(Message node) {
							return true;
						}
						...
						public void onWebSocketConnect(Session session) {
							...
							Stream<Message> receiving = Streams.defer(() -> strings)        /* items come in when we publish strings  */
